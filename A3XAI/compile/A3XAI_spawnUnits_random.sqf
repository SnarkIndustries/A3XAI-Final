...
private ["_patrolDist","_trigger","_totalAI","_minAI","_addAI","_unitLevel","_unitGroup","_playerPos","_spawnPos","_startTime","_baseDist","_extraDist","_spawnDist","_thisList" ,"_spawnPosSelected","_spawnChance","_firstActualPlayer","_moveMode"];
...
_spawnChance = if ((count _this) > 5) then {_this select 5} else {1};
_unitLevel = if ((count _this) > 6) then {_this select 6} else {1};		//(Optional) Select the item probability table to use
_numGroups = if ((count _this) > 7) then {_this select 7} else {1};		//(Optional) Number of groups of x number of units each to spawn
...
try {
	...
	if (_spawnChance call A3XAI_chance) then {
		...
		if !(_spawnPos isEqualTo []) then {
			...
			_unitGroup = [_totalAI,grpNull,"random",_spawnPos,_trigger,_unitLevel,"FULL",true] call A3XAI_spawnGroup;
			...
			0 = [_unitGroup,_triggerPos,_patrolDist,"LIMITED"] spawn A3XAI_BIN_taskPatrol;
			...
			if (!(_trigger getVariable ["initialized",false])) then {
				0 = [2,_trigger,[_unitGroup],_patrolDist,_unitLevel,[_minAI,_addAI]] call A3XAI_initializeTrigger;
			};
