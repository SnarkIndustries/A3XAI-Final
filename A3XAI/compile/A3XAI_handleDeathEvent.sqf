...
private ["_victim", "_killer", "_vehicle", "_unitGroup", "_victimPos", "_unitsAlive", "_groupIsEmpty", "_unitType", "_unitLevel", "_groupSize", "_fnc_deathHandler", 
"_launcherItem", "_bodyPosEmpty", "_collisionKilled"];
...
if !(isNull _victim) then {
	{
		if (_x in A3XAI_launcherTypes) exitWith {
			if (_x in (weapons _victim)) then {
				...
				} forEach (_victimPos nearObjects ["WeaponHolderSimulated",10]);
			};
		};
	...
	if !(isNull _vehicle) then {
		_bodyPosEmpty = _victimPos findEmptyPosition [0,2,DEFAULT_UNIT_CLASSNAME];
	...
	if (isDedicated) then {
		if ((isPlayer _killer) && {_unitGroup getVariable ["ReinforceAvailable",false]} && {(missionNamespace getVariable [format ["A3XAI_airReinforcementSpawnChance%1",_unitLevel],0]) call A3XAI_chance}) then {
			...
			_nul = [_victimPos,_killer,_unitLevel] spawn A3XAI_spawn_reinforcement;
		};
