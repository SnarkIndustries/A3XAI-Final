...
private ["_vehicle", "_vehicleType", "_spawnParams","_release"];

_vehicle = _this select 0;
_release = _this select 1;
...
if (isDedicated) then {
	...
	A3XAI_respawnVehicle_PVS = [_vehicle,_release];
	publicVariableServer "A3XAI_respawnVehicle_PVS";
};

if (A3XAI_vehiclesAllowedForPlayers && {alive _vehicle} && {_release}) then {
