...
"_unitType","_spawnChance","_playerDistNoLOS","_playerDistWithLOS","_isStandardGroup"];
...
_isStandardGroup = ((_unitType find "custom") isEqualTo -1);
_playerDistWithLOS = if (_isStandardGroup) then {PLAYER_DISTANCE_WITH_LOS_STATIC} else {PLAYER_DISTANCE_WITH_LOS_STATIC_CUSTOM};
_playerDistNoLOS = if (_isStandardGroup) then {PLAYER_DISTANCE_NO_LOS_STATIC} else {PLAYER_DISTANCE_NO_LOS_STATIC_CUSTOM};
...
_aiGroup = [_totalAI,_unitGroup,_unitType,_spawnPos,_trigger,_unitLevelEffective,"LIMITED",_checkPos] call A3XAI_spawnGroup;
if (isNull _unitGroup) then {diag_log format ["A3XAI Error: Respawned group at %1 was null group. New group reassigned: %2.",_trigger getVariable ["TriggerText","Unknown Trigger"],_aiGroup]; _unitGroup = _aiGroup};
...
if (A3XAI_debugLevel > 0) then {diag_log format["A3XAI Debug: %1 AI units respawned for group %2 (unitLevel %3) at %4 in %5 seconds.",_totalAI,_unitGroup,_unitLevelEffective,(_trigger getVariable ["TriggerText","Unknown Trigger"]),diag_tickTime - _startTime];};
