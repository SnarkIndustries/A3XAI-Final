...
if !((_unitGroup getVariable ["unitType",""]) isEqualTo "air_reinforce") then {[_vehicle,true] call A3XAI_respawnAIVehicle;};
...
if !(surfaceIsWater (getPosASL _vehicle)) then {
	...
	[_unitGroup,"aircrashed"] call A3XAI_setUnitType;	//Recategorize group as "aircrashed" to prevent AI inventory from being cleared since death is considered suicide.
	{
		if (alive _x) then {
