...
private ["_unitGroup","_targetPlayer","_startPos","_chaseDistance","_enableHCReady","_nearBlacklistedAreas","_targetPlayerPos","_originalSpeed"];
...
_originalSpeed = speedMode _unitGroup;
...
if ((_startPos distance _targetPlayerPos) < _chaseDistance) then {
	...
	if !(isNull _unitGroup) then {
		...
		if ((speedMode _unitGroup) != _originalSpeed) then {
			_unitGroup setSpeedMode _originalSpeed;
		};
