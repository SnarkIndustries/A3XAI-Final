...
while {(count A3XAI_respawnQueue) > 0} do {
	...
	if (A3XAI_currentFPS > A3XAI_minFPS) then {
				...
				call {
					if (_mode isEqualTo 0) exitWith {
						...
						if ((_unitGroup in _grpArray) && {((_unitGroup getVariable ["GroupSize",0]) < 1)}) then {
							if (_trigger in A3XAI_activePlayerAreas) then {
								...
								if ((A3XAI_debugLevel > 0) && {!_respawned}) then {diag_log format ["A3XAI Debug: No units were respawned for group %1 at %2. Group %1 reinserted into respawn queue.",_unitGroup,(_trigger getVariable ["TriggerText","Unknown Trigger"])];};
							} else {
								//Trigger is inactive (despawned or deleted) so clean up group instead
								if (A3XAI_debugLevel > 0) then {diag_log format ["A3XAI Debug: Spawn area %1 has already been despawned. Cleaning up group %2.",_trigger getVariable ["TriggerText","Unknown Trigger"],_unitGroup]};
					...
					diag_log format ["A3XAI Error: Invalid respawn mode %1 for group %2!",_mode,_unitGroup];
				};
				A3XAI_respawnQueue deleteAt _forEachIndex;
		...
		diag_log format ["A3XAI Debug: %1: Current server FPS %2 is lower than minimum threshold of %3.",__FILE__,A3XAI_currentFPS,A3XAI_minFPS];
		_minDelay = 30;
	};
	if (_minDelay > -1) then {
...
if (A3XAI_debugLevel > 0) then {diag_log "A3XAI Debug: Respawn queue is empty. Exiting respawn handler.";};
