	...
	try {
		...
		_currentTime = diag_tickTime;
		{
			if ((isPlayer _x) && {((typeOf _x) in [PLAYER_UNITS])}) then {
				...
				if ((_playerUID select [0,2]) != "HC") then {
		...
		if (_allPlayers isEqualTo []) then {
			throw "A3XAI Debug: No players online. Dynamic spawn manager is entering waiting state.";
		};
		
		if (A3XAI_currentFPS < A3XAI_minFPS) then {
			throw format ["A3XAI Debug: Current server FPS %1 is lower than minimum threshold of %2. Dynamic spawn manager is entering waiting state.",A3XAI_currentFPS,A3XAI_minFPS];
		};
		
		_activeDynamicSpawns = (count A3XAI_dynamicTriggerArray);
		...
		while {_allPlayers = _allPlayers - [objNull]; (((_maxSpawnsPossible - _activeDynamicSpawns) > 0) && {!(_allPlayers isEqualTo [])})} do {	//_spawns: Have we created enough spawns? _allPlayers: Are there enough players to create spawns for?
			...
			if (alive _player) then {
				...
				if (_spawnChance call A3XAI_chance) then {
					...
					_trigger = TRIGGER_OBJECT createVehicleLocal _playerPos;
					...
					_trigger setVariable ["TriggerText",(format ["Dynamic Spawn (Triggered by: %1)",_playername])];
					...
					if (A3XAI_enableDebugMarkers) then {
						...
						_marker setMarkerSize [TRIGGER_SIZE_DYNAMIC, TRIGGER_SIZE_DYNAMIC];
						_marker setMarkerAlpha 0;
						_trigger setVariable ["MarkerName",_marker];
					};
