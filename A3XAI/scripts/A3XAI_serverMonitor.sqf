...
while {true} do {
	...
	if ((_currentTime - _checkRandomSpawns) > MONITOR_CHECK_RANDSPAWN_FREQ) then {
		if (scriptDone _cleanupRandomSpawns) then {
			...
			_cleanupRandomSpawns = [_currentTime] spawn {
				_currentTime = _this select 0;
				A3XAI_randomTriggerArray = A3XAI_randomTriggerArray - [objNull];
				{
					if (((_x getVariable ["GroupArray",[]]) isEqualTo []) && {(_currentTime - (_x getVariable ["timestamp",_currentTime])) > RANDSPAWN_EXPIRY_TIME}) then {
						...
						deleteVehicle _x;
						A3XAI_randomTriggerArray deleteAt _forEachIndex;
					};
					if ((_forEachIndex % 3) isEqualTo 0) then {uiSleep 0.05};
				} forEach A3XAI_randomTriggerArray;
				_spawnsAvailable = A3XAI_maxRandomSpawns - (count A3XAI_randomTriggerArray);
	...
	if ((A3XAI_monitorReportRate > 0) && {((_currentTime - _monitorReport) > A3XAI_monitorReportRate)}) then {
		...
		diag_log format ["[A3XAI Monitor] [Uptime:%1:%2:%3][FPS:%4][Groups:%5][Respawn:%6][HC:%7]",_uptime select 0, _uptime select 1, _uptime select 2,round A3XAI_currentFPS,_activeGroupAmount,(count A3XAI_respawnQueue),A3XAI_HCIsConnected];
		diag_log format ["[A3XAI Monitor] [Static:%1][Dynamic:%2][Random:%3][Air:%4][Land:%5][UAV:%6][UGV:%7]",(count A3XAI_staticTriggerArray),(count A3XAI_dynamicTriggerArray),(count A3XAI_randomTriggerArray),A3XAI_curHeliPatrols,A3XAI_curLandPatrols,A3XAI_curUAVPatrols,A3XAI_curUGVPatrols];
		_monitorReport = _currentTime;
	};
