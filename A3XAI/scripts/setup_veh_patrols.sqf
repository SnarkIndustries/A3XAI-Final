...
diag_log format ["Debug: FPS Limit %1",A3XAI_minFPS];
		...
		for "_i" from 1 to _maxHelis do {
			if (A3XAI_currentFPS < A3XAI_minFPS) then {
				if (A3XAI_debugLevel > 0) then {
					diag_log format ["A3XAI Debug: Air Patrol spawning paused. Waiting for server FPS to be above minimum set value (Current: %1. Minimum: %2)",A3XAI_currentFPS,A3XAI_minFPS];
				};
...
if (A3XAI_maxLandPatrols > 0) then {
	_nul = [] spawn {
		...
		for "_i" from 1 to _maxVehicles do {
			if (A3XAI_currentFPS < A3XAI_minFPS) then {
				if (A3XAI_debugLevel > 0) then {
					diag_log format ["A3XAI Debug: Land Patrol spawning paused. Waiting for server FPS to be above minimum set value (Current: %1. Minimum: %2)",A3XAI_currentFPS,A3XAI_minFPS];
				};
				waitUntil {uiSleep 15; A3XAI_currentFPS > A3XAI_minFPS};
			};
...
if (A3XAI_maxUAVPatrols > 0) then {
	_nul = [] spawn {
		...
		for "_i" from 1 to _maxVehicles do {
			if (A3XAI_currentFPS < A3XAI_minFPS) then {
				if (A3XAI_debugLevel > 0) then {
					diag_log format ["A3XAI Debug: UAV Patrol spawning paused. Waiting for server FPS to be above minimum set value (Current: %1. Minimum: %2)",A3XAI_currentFPS,A3XAI_minFPS];
				};
				waitUntil {uiSleep 15; A3XAI_currentFPS > A3XAI_minFPS};
			};
...
if (A3XAI_maxUGVPatrols > 0) then {
	_nul = [] spawn {
		...
		for "_i" from 1 to _maxVehicles do {
			if (A3XAI_currentFPS < A3XAI_minFPS) then {
				if (A3XAI_debugLevel > 0) then {
					diag_log format ["A3XAI Debug: UGV Patrol spawning paused. Waiting for server FPS to be above minimum set value (Current: %1. Minimum: %2)",A3XAI_currentFPS,A3XAI_minFPS];
				};
				waitUntil {uiSleep 15; A3XAI_currentFPS > A3XAI_minFPS};
			};
