...
_nul = [] spawn {
	A3XAI_defaultTrigger = TRIGGER_OBJECT createVehicleLocal ([configFile >> "CfgWorlds" >> worldName,"centerPosition",[0,0,0]] call BIS_fnc_returnConfigEntry);
	...
	A3XAI_defaultTrigger setVariable ["TriggerText","Default Trigger Object"];
...
if (A3XAI_generateDynamicMedical) then {
	_lootlist = [] execVM format ['%1\scripts\A3XAI_buildMedicalList.sqf',A3XAI_directory];
	waitUntil {uiSleep 0.05; scriptDone _lootlist};
};
...
if (A3XAI_enableStaticSpawns) then {
	if (A3XAI_useManualStaticSpawns && {!(A3XAI_manualStaticSpawnLocations isEqualTo [])}) then {
		_staticSpawns = [] execVM format ["%1\scripts\A3XAI_generateStaticSpawnsManual.sqf",A3XAI_directory]; //sqf to be created
	} else {
		_staticSpawns = [] execVM format ["%1\scripts\A3XAI_generateStaticSpawnsAuto.sqf",A3XAI_directory];
	};
};
...
//Start universal spawn manager
_spawnManager = [] execVM format ['%1\scripts\A3XAI_spawnManagerUniversal.sqf',A3XAI_directory];
