...
_allUnits = +allUnits;
if (({if ((side _x) isEqualTo PLAYER_GROUP_SIDE) exitWith {1}} count _allUnits) isEqualTo 0) then {createCenter PLAYER_GROUP_SIDE};
if (({if ((side _x) isEqualTo A3XAI_side) exitWith {1}} count _allUnits) isEqualTo 0) then {createCenter A3XAI_side};
